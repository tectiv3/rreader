...
const localUncategorizedFeeds = ref([])
...
async function createCategory() {
    ...
    try {
        ...
        showError(e.response?.data?.message || 'Failed to create category')
    }
}
...
async function saveCategory(category) {
    ...
    try {
        ...
        showError(e.response?.data?.message || 'Failed to rename category')
    }
}
...
async function saveOrder(cats) {
    try {
        await axios.post('/api/categories/reorder', {
            order: cats.map(c => c.id),
        })
    } catch (e) {
        showError(e.response?.data?.message || 'Failed to reorder categories')
        await refetchData()
    }
}
...
async function confirmMoveFeed(feed) {
    try {
        await axios.put(`/api/feeds/${feed.id}`, {
            title: feed.title,
        ...
        showError(e.response?.data?.message || 'Failed to move feed')
    }
}
...
async function confirmUnsubscribe(feed) {
    try {
        await axios.delete(`/api/feeds/${feed.id}`)
        unsubscribingFeedId.value = null
        await refetchData()
    } catch (e) {
        showError(e.response?.data?.message || 'Failed to unsubscribe')
    }
}
...
async function reenableFeed(feed) {
    try {
        await axios.post(`/api/feeds/${feed.id}/reenable`)
        await refetchData()
    } catch (e) {
        showError(e.response?.data?.message || 'Failed to reenable feed')
    }
}
</script>
...
<template>
    ...
    <div v-else class="mx-auto max-w-2xl px-4 py-6 space-y-8">
        <!-- Create Category -->
        <section>
            <h2
                class="text-sm font-semibold uppercase tracking-wider text-neutral-600 dark:text-neutral-500 mb-3">
                New Category
            </h2>
        ...
        <section v-for="(category, catIndex) in localCategories" :key="category.id">
            <div class="flex items-center gap-2 mb-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <template v-else>
                        <h2
                            class="text-sm font-semibold uppercase tracking-wider text-neutral-600 dark:text-neutral-500">
                            {{ category.name }}
                        </h2>
                    </template>
                </div>
                ...
                <!-- Category actions -->
                <div
                    v-if="editingCategoryId !== category.id && deletingCategoryId !== category.id"
                    class="flex gap-1">
                    <button
            ...
            <!-- Delete confirmation -->
            <div
                v-if="deletingCategoryId === category.id"
                class="mb-3 rounded-lg border border-red-800/50 bg-red-950/50 p-4">
            ...
            <div class="space-y-1">
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div v-else-if="movingFeedId === feed.id" class="flex-1 min-w-0">
                            <div class="flex gap-2 items-center">
                                ...
                                <button
                                    @click="confirmMoveFeed(feed)"
                                    class="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700 transition-colors">
                                    Move
                                </button>
                                <button
                                    @click="cancelMoveFeed"
                                    class="rounded-lg px-3 py-1.5 text-xs text-neutral-500 dark:text-neutral-400 hover:text-neutral-800 dark:hover:text-neutral-200 transition-colors">
                                    Cancel
                                </button>
                            </div>
                        ...
                        <template v-else>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-1.5">
                                    <p
                                        class="text-sm text-neutral-800 dark:text-neutral-200 truncate">
                                        {{ feed.title }}
                                    </p>
                                    <svg
                    ...
                    <div
                        v-if="unsubscribingFeedId === feed.id"
                        class="mt-3 rounded-lg border border-red-800/50 bg-red-950/50 p-3">
                        <p class="text-sm text-red-300 mb-2">
                            Unsubscribe from "{{ feed.title }}"? All articles will be removed.
                        </p>
                        <div class="flex gap-2">
                            <button
                ...
                <p
                    v-if="category.feeds.length === 0"
                    class="px-4 py-2 text-sm text-neutral-600 dark:text-neutral-500 italic">
        ...
        <!-- Empty state -->
        <div
            v-if="localCategories.length === 0 && localUncategorizedFeeds.length === 0"
            ...
            <p class="mt-3 text-neutral-600 dark:text-neutral-500">
