        ...
        <nav class="fixed bottom-0 inset-x-0 z-40 border-t border-slate-800 bg-slate-900/95 backdrop-blur supports-[backdrop-filter]:bg-slate-900/80 lg:hidden pb-safe transition-transform duration-300"
            ...
            <div class="flex h-14 items-center justify-around px-2">
                ...
                <button
                    @click="navigateTo({})"
                    class="flex flex-col items-center justify-center gap-0.5 rounded-lg px-3 py-1.5 transition-colors"
                    :class="!page.props.activeFilter && !page.props.activeFeedId && !page.props.activeCategoryId && page.url.startsWith('/articles') && !page.url.startsWith('/articles/search') ? 'text-blue-500' : 'text-slate-400 hover:text-slate-200'"
                ...
                <button
                    ...
                    class="flex flex-col items-center justify-center gap-0.5 rounded-lg px-3 py-1.5 transition-colors"
                    :class="page.url.startsWith('/feeds/create') ? 'text-blue-500' : 'text-slate-400 hover:text-slate-200'"
                    aria-label="Add feed"
                >
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <button
                    ...
                    class="flex flex-col items-center justify-center gap-0.5 rounded-lg px-3 py-1.5 transition-colors"
                    :class="page.url.startsWith('/articles/search') ? 'text-blue-500' : 'text-slate-400 hover:text-slate-200'"
                    aria-label="Search"
                >
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
