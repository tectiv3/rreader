...
<template>
    ...
    <template v-if="!persistent">
        ...
        <Transition name="drawer">
            <div
                v-if="open"
                class="fixed inset-y-0 left-0 z-50 flex w-72 flex-col bg-slate-50 dark:bg-slate-900 shadow-xl"
                role="dialog"
                aria-label="Navigation sidebar"
            >
                <!-- Drawer header -->
                <div class="flex h-14 items-center justify-between border-b border-slate-200 dark:border-slate-800 px-4">
                    <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">RReader</h2>
                    <div class="flex items-center gap-1">
                        <Link
                            :href="route('feeds.manage')"
                            class="rounded-lg px-2.5 py-1.5 text-xs font-medium text-blue-400 hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-blue-300 transition-colors"
                        >
                            Edit
                        </Link>
                        <button
                ...
                <nav class="flex-1 overflow-y-auto py-2" aria-label="Feed navigation">
                    ...
                    <div class="px-3 pb-2">
                        ...
                        <button
                            ...
                            :class="activeFilter === 'today' ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-800/50'"
                        >
                            ...
                            <span v-if="sidebar.todayCount > 0" class="text-xs text-slate-600 dark:text-slate-500">{{ sidebar.todayCount }}</span>
                        </button>
                        ...
                        <button
                            ...
                            class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors cursor-pointer"
                            :class="activeFilter === 'read_later' ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-800/50'"
                        >
                            ...
                            <span v-if="sidebar.readLaterCount > 0" class="text-xs text-slate-600 dark:text-slate-500">{{ sidebar.readLaterCount }}</span>
                        </button>
                    </div>

                    <div class="mx-3 border-t border-slate-200 dark:border-slate-800" />

                    <!-- Feeds section -->
                    <div class="px-3 pt-3">
                        <div class="flex items-center justify-between px-3 pb-2">
                            <h3 class="text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-500">Feeds</h3>
                        </div>
                        ...
                        <button
                            @click="navigateTo({})"
                            class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors cursor-pointer"
                            :class="isActiveAll() ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-800/50'"
                        >
                        ...
                        <div v-for="category in sidebar.categories" :key="category.id" class="mt-0.5">
                            <div
                                ...
                                :class="activeCategoryId === category.id ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-800/50'"
                            >
                                <button
                                ...
                                >
                                    ...
                                    <span v-if="category.unread_count > 0" class="text-xs text-slate-600 dark:text-slate-500">{{ category.unread_count }}</span>
                                </button>
                            </div>
                            ...
                            <div v-if="isCategoryExpanded(category.id)" class="ml-4">
                                <button
                                    ...
                                    :class="activeFeedId === feed.id ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-200/50 dark:hover:bg-slate-800/50 hover:text-slate-700 dark:hover:text-slate-300'"
                                >
                                    <img
                        ...
                        <div v-if="sidebar.uncategorizedFeeds.length > 0" class="mt-0.5">
                            <button
                                ...
                                :class="activeFeedId === feed.id ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-800/50'"
                            >
                                <img
                                    v-if="feed.favicon_url"
                                    :src="feed.favicon_url"
                ...
                <div class="border-t border-slate-200 dark:border-slate-800 p-3 space-y-2">
                    ...
                    <Link
                        ...
                        class="flex w-full items-center justify-center gap-2 rounded-lg bg-slate-200 dark:bg-slate-800 px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors"
                    >
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <template v-else>
        <aside
            class="flex flex-col border-r border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 transition-all duration-200"
            :class="collapsed ? 'w-16' : 'w-72'"
            aria-label="Navigation sidebar"
        >
            <!-- Sidebar header -->
            <div class="flex h-14 items-center border-b border-slate-200 dark:border-slate-800 px-3" :class="collapsed ? 'justify-center' : 'justify-between'">
                <h2 v-if="!collapsed" class="text-lg font-semibold text-slate-900 dark:text-slate-100 truncate">RReader</h2>
                <div class="flex items-center gap-1">
                    <Link
                        v-if="!collapsed"
                        :href="route('feeds.manage')"
            ...
            <nav class="flex-1 overflow-y-auto py-2" aria-label="Feed navigation">
                ...
                <div class="px-3 pb-2">
                    ...
                    <button
                        ...
                        :class="[
                            ...
                            activeFilter === 'today' ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-800/50',
                        ]"
                    ...
                    <button
                        ...
                        :class="[
                            collapsed ? 'justify-center px-0 py-2.5' : 'px-3 py-2.5',
                            activeFilter === 'read_later' ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-800/50',
                        ]"
                ...
                <div class="px-3 pt-3">
                    ...
                    <button
                        ...
                        :class="[
                            collapsed ? 'justify-center px-0 py-2.5' : 'px-3 py-2.5',
                            isActiveAll() ? 'bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-800/50',
                        ]"
                        :title="collapsed ? 'All Feeds' : undefined"
                    >
                        <svg class="h-5 w-5 shrink-0 text-slate-500 dark:text-slate-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12.75 19.5v-.75a7.5 7.5 0 00-7.5-7.5H4.5m0-6.75h.75c7.87 0 14.25 6.38 14.25 14.25v.75M6 18.75a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
                        </svg>
                        <template v-if="!collapsed">
                            <span class="flex-1 text-left">All</span>
            ...
            <div class="border-t border-slate-200 dark:border-slate-800 p-3 space-y-2">
                ...
                <Link
                    :href="route('settings.index')"
                    class="flex w-full items-center rounded-lg bg-slate-200 dark:bg-slate-800 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors"
                    :class="collapsed ? 'justify-center p-2.5' : 'justify-center gap-2 px-4 py-2.5'"
