...
function feedUnreadCount(feedId) {
    // Find feed in sidebar data (server-provided counts)
    for (const cat of props.sidebar?.categories || []) {
        const feed = cat.feeds.find(f => f.id === feedId)
        if (feed) return feed.unread_count || 0
    }
    const uncatFeed = (props.sidebar?.uncategorizedFeeds || []).find(f => f.id === feedId)
    return uncatFeed?.unread_count || 0
}

function categoryUnreadCount(category) {
    return category.unread_count || 0
}
</script>
...
<template>
    ...
    <template v-else>
        <aside
            class="flex flex-col border-r border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 transition-all duration-200 shrink-0"
            :class="collapsed ? 'w-16' : 'w-72'"
            aria-label="Navigation sidebar">
            <!-- Sidebar header (h-11 to align with article list header) -->
            <div
                class="flex h-11 items-center border-b border-neutral-200 dark:border-neutral-800 px-3"
            ...
            <nav class="flex-1 overflow-y-auto py-2" aria-label="Feed navigation">
                    ...
                    <button
                        ...
                        :class="
                            activeFilter === 'recently_read'
                                ...
                                : 'text-neutral-700 dark:text-neutral-300 hover:bg-neutral-200/50 dark:hover:bg-neutral-800/50'
                        "
                        :title="collapsed ? 'Recently Read' : undefined">
                        <svg
                ...
                <!-- Feeds section (hidden when collapsed) -->
                ...
                <!-- Collapsed: All feeds icon -->
                <div v-if="collapsed" class="px-2 pt-3">
                    <button
                        @click="navigateTo({})"
                        class="flex w-full items-center justify-center rounded-lg px-3 py-2.5 text-sm transition-colors cursor-pointer"
                        :class="
                            isActiveAll()
                                ? 'bg-neutral-200 dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100'
                                : 'text-neutral-700 dark:text-neutral-300 hover:bg-neutral-200/50 dark:hover:bg-neutral-800/50'
            ...
            <!-- Sidebar footer -->
            <div
                class="border-t border-neutral-200 dark:border-neutral-800 p-2"
                :class="collapsed ? 'space-y-1' : 'space-y-2 p-3'">
                <button
                    @click="openAddFeedFromDrawer()"
                    class="flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 text-sm font-medium text-white hover:bg-blue-700 transition-colors cursor-pointer"
                    :class="collapsed ? 'p-2.5' : 'px-4 py-2.5'"
                ...
                <button
                    @click="goToSettings()"
                    class="flex w-full items-center justify-center gap-2 rounded-lg bg-neutral-200 dark:bg-neutral-800 text-sm font-medium text-neutral-700 dark:text-neutral-300 hover:bg-neutral-300 dark:hover:bg-neutral-700 transition-colors cursor-pointer"
                    :class="collapsed ? 'p-2.5' : 'px-4 py-2.5'"
