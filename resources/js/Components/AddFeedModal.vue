...
// Feed tab state (step 1)
const url = ref('')
const urlError = ref('')
const isSearching = ref(false)

// Feed tab state (step 2)
...
// Save article tab state
const articleUrl = ref('')
const isSavingArticle = ref(false)
const saveError = ref('')

const urlInput = ref(null)
const articleUrlInput = ref(null)
...
watch(
    ...
    newVal => {
        if (newVal) {
            ...
            if (props.initialUrl) {
                articleUrl.value = props.initialUrl
            }
            nextTick(() => {
                if (activeTab.value === 'article') {
                    articleUrlInput.value?.focus()
                } else {
...
async function saveArticle() {
    ...
    try {
        const { data } = await axios.post('/api/articles/save-url', {
            url: articleUrl.value,
        })

        if (data.duplicate) {
            success('Article already saved -- marked as read later')
        } else {
        ...
        saveError.value = e.response?.data?.message || 'Failed to save article'
    } finally {
        isSavingArticle.value = false
    }
}
                    ...
                    <div
                        ...
                        <div
                            ...
                            <h2
                                ...
                                {{ activeTab === 'feed' ? 'Add Feed' : 'Save Article' }}
                            </h2>
                            <button
                                @click="close"
                        ...
                        <div class="p-5">
                            <!-- Tab switcher -->
                            <div
                                class="mb-4 flex rounded-lg bg-neutral-200 dark:bg-neutral-800 p-0.5">
                                <button
                            ...
                            <div v-if="activeTab === 'feed'">
                                ...
                                <div v-else>
                                    ...
                                    <form @submit.prevent="subscribe">
                                        ...
                                        <InputError
                                            class="mb-3"
                                            :message="subscribeErrors.feed_url" />
                            ...
                            <div v-else-if="activeTab === 'article'">
                                <form @submit.prevent="saveArticle">
                                    <div>
                                        ...
                                        <InputError class="mt-2" :message="saveError" />
                                    </div>

                                    <button
                                        type="submit"
                                        class="mt-4 w-full inline-flex items-center justify-center rounded-md border border-transparent bg-blue-600 px-4 py-3 text-sm font-semibold uppercase tracking-widest text-white transition duration-150 ease-in-out hover:bg-blue-500 focus:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-neutral-900 active:bg-blue-700"
                                        :class="{ 'opacity-25': isSavingArticle }"
